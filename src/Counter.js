...
export const Counter = () => {
    ...
    function countDown() {
      interval.current = setInterval(() => {
        ...
        if (secondCounter.current === 0 && minuteCounter.current === 0) {
          countStudySessions.current > countRestSessions.current
            ? countRestSessions.current++
            : countStudySessions.current++;
          playSound();
          ...
          minuteCounter.current =
            countStudySessions.current > countRestSessions.current
              ? restTimeRef.current.value
              : studyTimeRef.current.value;
          setSeconds(secondCounter.current);
          setMinutes(minuteCounter.current);
          paused.current = true;
        }
  ...
  const disableStudyOptions =
    started || countStudySessions.current > countRestSessions.current;
  const disableRestOptions =
    started ||
    (countRestSessions.current === countStudySessions.current &&
      countStudySessions.current !== 0);
  ...
  return (
    <div>
      ...
      <button onClick={handleResetTimer} className="reset-timer">
        Reset timer
      </button>
      ...
      <select
        ...
        ref={restTimeRef}
        disabled={disableRestOptions}
      >
        <option value={5} key={5}>
          5
        </option>
        <option disabled={!moreRestEnable} value={10} key={10}>
          10
        </option>
      </select>
      ...
      <br />
      {moreRestEnable ? (
        <label className="more-rest">you can Choose 10 minute option now</label>
      ) : (
        <label className="session-count">
          Study {4 - countStudySessions.current} more sessions to allow 10
          minute rest option
        </label>
      )}
      <br />
